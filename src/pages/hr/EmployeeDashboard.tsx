...
const EmployeeDashboard = () => {
  const [isDeleteEmployeeClicked, setIsDeleteEmployeeClicked] =
    useState<boolean>(false);
  const [employeeExists, setEmployeeExists] = useState<boolean>(true);

  const { openedEmployee, handleEmployeeDashboardClose, getEmployeeById } =
    useUser();
  const { employeeId } = useParams();

  useEffect(() => {
    getEmployeeById(employeeId).then((res: any) => {
      if (res.data === "") {
  ...
  if (!openedEmployee) return <LoadingComponent />;
      ...
      <Grid container direction="row" justifyContent="center" spacing={2}>
        <Grid item container direction="column" lg={12}>
          <Stack direction="row" justifyContent="center" spacing={2} mb={3}>
            <Button onClick={handleEmployeeDashboardClose} variant="contained">
              Back
            </Button>
            <Button
              onClick={() => setIsDeleteEmployeeClicked(true)}
              variant="contained"
              disabled={openedEmployee.hasLeft}
            >
              Delete
            </Button>
          </Stack>
