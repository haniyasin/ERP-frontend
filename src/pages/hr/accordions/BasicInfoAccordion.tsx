...
const BasicInfoAccordion = ({ employeeLeft }: BasicInfoAccordionProps) => {
  ...
  const [isEditEmployeeClicked, setIsEditEmployeeClicked] = useState<boolean>(false);
  ...
  const joinedDepartmentIds = openedEmployee?.departments.map((department: Department) => department.id);
  const imageUrl = extractProfilePicture(openedEmployee?.picture?.data);

  const onSubmit = (data: User) => {
    if (!data.departments) return;
    const inputData = { ...data, departments: data.departments.map(dep => { return { id: dep } })}
    editEmployee(inputData)
      .then((res: boolean) => {
        if (res) {
  ...
  return (
    <Accordion elevation={0} defaultExpanded={false}>
            ...
            <AccordionDetails>
              <UploadPictureModal open={isModalOpen} onClose={() => setIsModalOpen(false)}/>
              <Form onSubmit={onSubmit} validationSchema={editEmployeeSchema}>
                <Grid container direction="row" spacing={1} marginLeft={6} marginTop={4}>
                  <Grid item container direction="column" lg={2} alignContent="flex-start">
                      <ProfilePicture
                        isHovered={!openedEmployee.hasLeft && isHovered }
                        imageUrl={imageUrl}
                        onHover={() => !openedEmployee.hasLeft && setIsHovered(prev => !prev)}
                  ...
                  <Grid item container direction="column" lg={4} alignContent="flex-end">
                    <Stack direction="column" spacing={0} width={"80%"}>
                      ...
                      <InputField
                        ...
                        label="Starting Date"
                        type="date"
                        defaultValue={formatDateToLocaleTime(openedEmployee.startingDate)}
                        readOnly={!isEditEmployeeClicked}
                      />
                    </Stack>
                  </Grid>
                  <Grid item container direction="column" lg={4} alignContent="flex-start">
                    <Stack direction="column" spacing={0} width={"80%"}>
                      <InputField
                        name="title"
                        label="Title"
                ...
                <Stack direction="row" justifyContent="center" spacing={2} mt={3}>
                  <Button onClick={() => setIsEditEmployeeClicked(prev => !prev)} variant='contained' disabled={employeeLeft}>Edit</Button>
                  <Button type="submit" variant='contained' disabled={!isEditEmployeeClicked || employeeLeft}>Submit</Button>
                </Stack>
              </Form>
            </AccordionDetails>
          </Accordion>
