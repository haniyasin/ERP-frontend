...
const NewBonusModal = ({ closeModal, isModalOpen }: NewBonusModalProps) => {
    ...
    const onSubmit = (data: Bonus) => {
      ...
      post("/bonuses", dataWithUserID, 'multipart/form-data')
          .then((res) => {
              if(res) {
                handleEmployeeDashboardOpen({...openedEmployee, bonuses: [...openedEmployee.bonuses, res.data]});
    ...
    return (
        <ModalBox open={isModalOpen} onClose={closeModal} width={400}>
            <Typography variant="h6" textAlign="center" marginBottom={2}>New Bonus</Typography>
