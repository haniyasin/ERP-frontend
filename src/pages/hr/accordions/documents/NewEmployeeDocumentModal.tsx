...
const NewEmployeeDocumentModal = ({ closeModal, isModalOpen }: NewBonusModalProps) => {
    const { post } = useHttp();
    ...
    const onSubmit = (data: EmployeeDocument) => {
      ...
      post("/employee/documents", dataWithUserID, 'multipart/form-data')
          .then((res) => {
              if(res) {
                handleEmployeeDashboardOpen({...openedEmployee, documents: [...openedEmployee.documents, res.data]});
    ...
    return (
        <ModalBox open={isModalOpen} onClose={closeModal} width={300}>
            <Typography variant="h6" textAlign="center" marginBottom={2}>New Document</Typography>
