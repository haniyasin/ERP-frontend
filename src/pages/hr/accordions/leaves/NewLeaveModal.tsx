...
const NewLeaveModal = ({ closeModal, isModalOpen }: NewLeaveModalProps) => {
    ...
    const onSubmit = (data: Leave) => {
      ...
      post("/leaves", dataWithUserID, 'multipart/form-data')
          .then((res) => {
              if(res) {
                handleEmployeeDashboardOpen({
                  ...openedEmployee,
                  leaves: [...openedEmployee.leaves, res.data],
                  leaveDaysLeft: res.data.type === "Paid" ? parseFloat((openedEmployee.leaveDaysLeft - getLeaveDaysCount(res.data)).toFixed(2)) : openedEmployee.leaveDaysLeft
                });
    ...
    return (
        <ModalBox open={isModalOpen} onClose={closeModal} width={400}>
            <Typography variant="h6" textAlign="center" marginBottom={2}>New Leave</Typography>
