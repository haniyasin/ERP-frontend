...
const SalaryAccordion = ({ employeeLeft }: SalaryAccordionProps) => {
  const [isNewSalaryModalOpen, setIsNewSalaryModalOpen] = useState<boolean>(false);
        ...
        {openedEmployee?.currentSalary && (
          <Container>
            ...
            <Typography variant="body1" sx={{fontWeight: 'bold' }}>
              Start Date: {formatDateToLocaleTime(openedEmployee.currentSalary.startDate)}
            </Typography>
          </Container>
        )}
        <Container sx={{ textAlign: 'center', marginBottom: '0.5rem' }}>
          <Button onClick={() => setIsNewSalaryModalOpen(true)} variant="contained" sx={{ marginTop: '1rem' }} disabled={employeeLeft}>
            New Salary
          </Button>
        </Container>
        <NewSalaryModal isModalOpen={isNewSalaryModalOpen} closeModal={() => setIsNewSalaryModalOpen(false)} />
        <SalariesList />
      </AccordionDetails>
    </Accordion>
  );
