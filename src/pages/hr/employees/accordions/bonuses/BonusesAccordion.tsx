...
const BonusesAccordion = ({ employeeLeft }: BonusesAccordionProps) => {
  const [isNewBonusModalOpen, setIsNewBonusModalOpen] =
    ...
    <Accordion elevation={0} defaultExpanded={false}>
      ...
      <AccordionDetails>
        <Container sx={{ textAlign: "center", marginBottom: "0.5rem" }}>
          <Button
            onClick={() => setIsNewBonusModalOpen(true)}
            variant="contained"
            sx={{ marginTop: "1rem" }}
        ...
        <NewBonusModal
          isModalOpen={isNewBonusModalOpen}
          closeModal={() => setIsNewBonusModalOpen(false)}
        />
        <BonusesList />
      </AccordionDetails>
    </Accordion>
